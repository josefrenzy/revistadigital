<footer class="page-footer font-small blue pt-4" style="background-color: #083d7d !important; color: #fff">
    <div class="container-fluid text-center text-md-left">
        <div class="row">
            <div class="col-md-3 mb-md-0 mb-3">
                <img src="{{ url('/images/utils/footer.png') }}" alt="Footer" style="max-width: 90%" />
            </div>
            <div class="col-md-3 mb-md-0 mb-3">
                <h5 class="text-uppercase"><strong>Horario</strong></h5>
                <p>Lunes a viernes de 9:00 AM a 6:00 PM</p>

                <h5 class="text-uppercase"><strong>EMAIL</strong></h5>
                <p>contacto@fg.com.mx</p>

            </div>
            <div class="col-md-3 mb-md-0 mb-3">
                <h5 class="text-uppercase"><strong>Oficinas Guadalajara</strong></h5>
                <p>
                    Av. de las Rosas #210<br>
                    Colonia Chapalita<br>
                    Guadalajara, Jal, Mex C.P. 44510<br>
                    Tel: (33) 33 030 100
                </p>
            </div>
            <div class="col-md-3 mb-md-0 mb-3">
                <h5 class="text-uppercase"><strong>OFICINA VALLARTA</strong></h5>
                <p>
                    Paseo de los cocoteros #85 Sur L-11<br>
                    Centro Comercial Paradise Village Plaza<br>
                    Nuevo Valarta, Nay, Mex CP: 63732<br>
                    Tel: 322 297 2095, Fax: 322 297 2007
                </p>
            </div>
        </div>
    </div>
    <div class="footer-copyright text-center py-3">© 2021 Copyright:
        <a class="aviso-privacidad" href="http://floresgaribay.mx/aviso-de-privacidad/"><strong>Aviso de Privacidad</strong></a>
    </div>
</footer>